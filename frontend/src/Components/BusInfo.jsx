...
const BusInfo = () => {
  ...
  // --------------------------------------------------
  // Handle Select Seats / Hide Seats
  // --------------------------------------------------
  const handleToggleSeats = async (busId, price) => {
    try {
      // Opening a different bus
      if (openBusId !== busId) {
        await fetchSeats(busId);
      ...
      dispatch(
        setToast({
          message: "Failed to fetch seats for selected bus",
          success: false
        })
      );
    }
  ...
  // --------------------------------------------------
  useEffect(() => {
    // Don't poll if no bus is currently open
    if (!openBusId) return;

    // Fetch immediately when polling starts
    fetchSeats(openBusId);
    ...
    // Stop polling when bus is closed/unmounted
    ...
    <div className="flex flex-col gap-4">
      {busList.map((bus) => {
        ...
        const price = Math.floor(
          bus.basePrice + bus.distance * 2
        );

        const seatUrgency = getSeatUrgency(
          bus.availableSeatsCount,
          bus.totalSeats
        );
            ...
            <div className="hidden md:flex md:items-center md:justify-between md:p-5 gap-4">
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="text-right min-w-[80px]">
                  ...
                  <div
                    className={`inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-md text-xs font-medium ${seatUrgency.bg} ${seatUrgency.color}`}
                  >
                ...
                <button
                  onClick={() =>
                    handleToggleSeats(bus._id, price)
                  }
            ...
            <div className="md:hidden p-4">
              ...
              <div className="flex items-center justify-between bg-slate-50 rounded-xl p-3 mb-3">
                <div className="text-center flex-1">
                  ...
                  <p className="text-xs text-[#4B5563] font-medium truncate">
                    {boarding}
                  </p>
                ...
                <div className="flex flex-col items-center px-2">
                  ...
                  <p className="text-[10px] font-semibold text-[#4B5563] mt-1">
                    {bus.distance} km
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-blue-50">
                <div>
                  ...
                  <div
                    className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-md text-xs font-medium ${seatUrgency.bg} ${seatUrgency.color}`}
                  >
                    {bus.availableSeatsCount} seats left
                  </div>
                </div>
